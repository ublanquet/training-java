  ...
  val browse = feed(feederColumn)
    ...
    .repeat(numberPage) {
      exec(http("Browse: Browse page: ${page},  pageSize: ${pageSize}, column: ${column}, order: ${order}")
        ...
        .formParam(config.getString("application.urls.param.page").toString(), "${page}")
        .formParam(config.getString("application.urls.param.pageSize").toString(), "${pageSize}")
        .formParam(config.getString("application.urls.param.column").toString(), "${column}")
        .formParam(config.getString("application.urls.param.order").toString(), "${order}")
        .check(status.is(200))
      )
        .pause(3, 10)
    }
